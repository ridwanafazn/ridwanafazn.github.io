...
export default function TechStack() {
  ...
  return (
    <section 
      id="about" 
      className="scroll-mt-32 py-24 px-6 md:px-12 lg:px-24 max-w-7xl mx-auto"
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 auto-rows-[180px]">
        ...
        >
          <div>
            ...
            <p className="text-sm text-slate-400 leading-relaxed">
              High-performance microservices and distributed systems. Focusing on memory safety and concurrency.
            </p>
          </div>
          <div className="flex flex-wrap gap-2 mt-4">
            {["Rust", "Golang", "C++", "gRPC", "MQTT", "Actix"].map((tag) => (
              <span key={tag} className="px-2 py-1 rounded bg-slate-800 text-xs text-slate-300 font-mono border border-slate-700">
                {tag}
              </span>
            ))}
          </div>
        ...
        >
          <div className="flex items-start justify-between">
            <div>
              <div className="h-10 w-10 rounded-lg bg-teal-500/10 flex items-center justify-center mb-4 text-teal-400">
                <Cloud size={20} />
              </div>
        ...
        >
          <div className="flex items-start justify-between">
            ...
            <div className="flex gap-3 text-slate-800 group-hover:text-pink-500/30 transition-colors">
              <Layout size={32} />
              <Smartphone size={32} />
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-2 mt-4">
             <span className="px-2 py-1 rounded bg-slate-800 text-xs text-slate-300 font-mono border border-slate-700">Next.js</span>
             <span className="px-2 py-1 rounded bg-slate-800 text-xs text-slate-300 font-mono border border-slate-700">Flutter</span>
             <span className="px-2 py-1 rounded bg-slate-800 text-xs text-slate-300 font-mono border border-slate-700">Tailwind</span>
             <span className="px-2 py-1 rounded bg-slate-800 text-xs text-slate-300 font-mono border border-slate-700">Framer Motion</span>
          </div>
        </motion.div>
